...
class AC_CF7_api_admin{
  ...
  public function __construct(){
    ...
    $this->textdomain = 'ac-cf7-intergration';

    $this->admin_notices = new AC_Admin_notices();
  ...
  public function verify_dependencies(){
    if( ! is_plugin_active('contact-form-7/wp-contact-form-7.php') ){
      $notice = array(
        'id'                  => 'cf7-not-active',
        'type'                => 'warning',
        'notice'              => __( 'ActiveCampaign & Contact Form 7 integrations requires CONTACT FORM 7 Plugin to be installed and active' ,$this->textdomain ),
  ...
  public function register_hooks(){
    ...
    add_action( 'wpcf7_before_send_mail', array( $this , 'ac_cf7_send_data_to_api' ) );
    ...
    add_action( "wpcf7_save_contact_form" ,array( $this , "ac_save_contact_form_details") , 10 , 1 );
  ...
  function wpcf7_integrations( $post ) {
    ...
    $wpcf7_api_data["base_url_2"]     = isset( $wpcf7_api_data["base_url_2"] ) ? $wpcf7_api_data["base_url_2"]         : '';
        ...
        <fieldset>
          ...
          <div class="cf7_row">
              <label for="wpcf7-sf-base_url">
                  <?php _e( 'Base url For Create Account' , $this->textdomain );?>
          ...
          <hr>
          <div class="cf7_row">
               <label for="wpcf7-sf-base_url_2">
                  <?php _e( 'Base url For Custom Field' , $this->textdomain );?>
                  <input type="text" id="wpcf7-sf-base_url_2" name="wpcf7-sf[base_url_2]" class="large-text" value="<?php echo $wpcf7_api_data["base_url_2"];?>" />
               </label>
          </div>
...
<hr/>
<textarea name="json_template_2" rows="12" dir="ltr" placeholder="<?php echo esc_attr( $json_placeholder );?>"><?php echo isset( $wpcf7_api_json_data_template_2 ) ? $wpcf7_api_json_data_template_2 : ""; ?></textarea>
      </div>
    </fieldset>
...
<?php
  ...
  public function qs_save_contact_form_details( $contact_form ){
    ...
    $properties['json_template_2']      = isset( $_POST["json_template_2"] ) ? $_POST["json_template_2"] : '';
  ...
  public function qs_cf7_send_data_to_api( $WPCF7_ContactForm ) {
    ...
    if( isset( $qs_cf7_data["send_to_api"] ) && $qs_cf7_data["send_to_api"] == "on" ){
      ...
      $record["base_url_2"] = $qs_cf7_data["base_url_2"];
      if( isset( $record["url"] ) && $record["url"] ){
        ...
        if( is_wp_error( $response ) ){
          ...
          $response = $this->send_customField( $record , $qs_cf7_data['debug_log'] , $qs_cf7_data['method'] , $record_type, $qs_cf7_data["basic_auth"], $qs_cf7_data["bearer_auth"] );
        }
      }
    }
  ...
  private function send_lead( $record , $debug = false , $method = 'GET' , $record_type = 'params', $basic_auth = null, $bearer_auth = null ){
    ...
    if( $method == 'GET' && ( $record_type == 'params' || $record_type == 'json' ) ){
      ...
      $args['headers']['Api-Token'] = '91d1f468ae22761090790f6bf8e7bb3155b3b6d7fb0c5462802621f648f84f0f9bb906ba';
      ...
      $this->contact_id = $json["contact"]["id"];
    }
    ...
    if( $debug ){
      ...
      update_post_meta( $this->post->id() , 'qs_cf7_api_debug_params' , $record );
      update_post_meta( $this->post->id() , 'qs_cf7_api_debug_result' , $record );
    }
  ...
  private function send_customField( $record , $debug = false , $method = 'GET' , $record_type = 'params', $basic_auth = null, $bearer_auth = null ){
    global $wp_version;
    $accountId  = $this->contact_id;
    $external = json_decode($record["fields"], true);
    $data = $external["contact"]["companyUrl"];
    $lead = "{\"fieldValue\":{\"contact\":\"{$accountId}\", \"field\":\"1\", \"value\":\"{$data}\"}}" ;
    $url  = $record["base_url_2"];
    ...
    if( $debug ){
      update_post_meta( $this->post->id() , 'qs_cf7_api_debug_url' , $record["base_url_2"] );
      update_post_meta( $this->post->id() , 'qs_cf7_api_debug_params' , $external["contact"]  );
      update_post_meta( $this->post->id() , 'qs_cf7_api_debug_result' , $result );
    }

    return do_action('after_qs_cf7_api_send_customField' , $result , $record );
